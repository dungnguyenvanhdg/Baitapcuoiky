...
public class NVD_Blockchain {
    public static ArrayList<VNPT_Dung> blockchain = new ArrayList<VNPT_Dung>();
    public static int difficulty = 5;
    public static float minimumTransaction = 0.1f;
    public static Store NVD_Kho; //Kho
    public static Store NVD_Cuahang; //Cửa hàng
    ...
    public static void main(String[] args) {
        ...
        genesisTransaction = new Transaction(coinbase.publicKey, NVD_Kho.publicKey, initBalanceA, null);
        ...
        VNPT_Dung genesis = new VNPT_Dung("0");
        genesis.addTransaction(genesisTransaction);
        addBlock(genesis);
        System.out.println("Số lượng VNPT-Net Router trong kho là : " + NVD_Kho.getBalance());
        System.out.print("Số lượng VNPT-Net Router của Cửa hàng đang còn : ");
        float initBalanceB = sc.nextFloat();
        genesisTransaction = new Transaction(coinbase.publicKey, NVD_Cuahang.publicKey, initBalanceB, null);
        genesisTransaction.generateSignature(coinbase.privateKey);
        genesisTransaction.transactionId = "0";
        genesisTransaction.outputs.add(new TransactionOutput(genesisTransaction.reciepient, genesisTransaction.value, genesisTransaction.transactionId)); //Thêm Transactions Output
        UTXOs.put(genesisTransaction.outputs.get(0).id, genesisTransaction.outputs.get(0));
        genesis.addTransaction(genesisTransaction);
        addBlock(genesis);
        System.out.println("Số lượng VNPT-Net Router của Cửa hàng đang còn là : " + NVD_Cuahang.getBalance());
        VNPT_Dung block1 = new VNPT_Dung(genesis.hash);
        boolean fail = true;
        while (fail){
            ...
            sendFund = NVD_Kho.sendFunds(NVD_Cuahang.publicKey, numberTransfer);
            if (sendFund==null){
                continue;
            }else{
                fail= false;
        ...
        System.out.println("Số lượng VNPT-Net Router trong kho hiện tại là : " + NVD_Kho.getBalance());
        System.out.println("Số lượng VNPT-Net Router của Cửa hàng hiện tại là : " + NVD_Cuahang.getBalance());
    }
    public static Boolean isChainValid() {
        VNPT_Dung currentBlock;
        VNPT_Dung previousBlock;
    ...
    public static void addBlock(VNPT_Dung newBlock) {
